...
int main( int argc, char *argv[] )
{
    ...
    if( ktime%( (fpulse.NewNt-1)/200 )==0 )
    {
        ...
        else
        {
            ...
            printf( "\n%.6d / %.7d        %.3e        %.3e        %.2d        %.6d     %s  %.3f %s %.3f %s", ktime, fpulse.NewNt, real(ConductionOccup[ktime]*g.dV  ), real(Coherence_Mom_Int[ktime])*g.dV, cs.gauge, rank, "Non-Shift BZ (",tkx5[ 0 ],",",tky5[ 0 ],")" );
