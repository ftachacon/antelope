...
int main( int argc, char *argv[] )
{
    ...
    string IntionMethod = "Simpson";//"Trapz";//       // "Trapz" or "Simpson" or "NoneTrapz"/"NoneSimpson"
    ...
    kmaxs0[0]       = pi/sqrt( 3. )/a0;      // Momentum grid max on x-dir a.u.
    ...
    complex cum_inter[Ngrad];
    complex cum_intra[Ngrad];
    ...
    {            
        ...
        for( itemp = 0; itemp < g.N[0] ; itemp++ )
        {
            ...
            der_nc( &tROmega5[0], &temp_pi5[0], &k_nc5[0] );
            ...
            der_nc( &tROmega5[1], &temp_pi5[1], &k_nc5[1] );
    ...
    if( ktime%( (fpulse.NewNt-1)/200 )==0 )
    {
        ...
        if ( g.k[1][jtemp-1] >= kyShift[0] & g.k[1][jtemp-1] <= kyShift[1] & fbz_shift == 1)
        {
            ...
            printf( "\n%.6d / %.7d        %.3e        %.3e        %.2d        %.6d     %s  %.3f %s %.3f %s", ktime, fpulse.NewNt, real(ConductionOccup[ktime]*g.dV  ), real(Coherence_Mom_Int[ktime])*g.dV, cs.gauge, rank, "Shifted BZ (",tkx5[ 0 ],",",tky5[ 0 ],")" );
    ...
    for( ktime = 1; ktime<fpulse.NewNt; ktime++ )
    {
        ...
        dxdt = real(inter_rad[0][ktime])*g.dV;
        //real( inter_rad[0][ktime+1] - inter_rad[0][ktime-1] )*g.dV/dt/2.;
        
        
        
        
        dydt = real(inter_rad[1][ktime])*g.dV;
        //real( inter_rad[1][ktime+1] - inter_rad[1][ktime-1] )*g.dV/dt/2.;
