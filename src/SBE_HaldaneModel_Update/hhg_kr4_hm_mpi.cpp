...
int main( int argc, char *argv[] )
{
    ...
    //cs.T2   = period0;
        ...
        fprintf(sparamout,"\n\n%s","#Parameters, No.cores, ellipticity,   ---,        ---,          ---,        ,    gauge2 ");         
        
        fprintf(sparamout,"\n          %d      %e      %e      %e      %e      %e      %d      \n\n", Nprocessors, ellip, 0., 0., 0., 0., gauge2 );
        fflush(sparamout );
    ...
    cout << "\nrank = " << rank << "  uses j = " << jstart << ";  to  j = " << jend << "  with Nprocesses = " << Nprocessors; 
        ...
        for( i = 0; i < g.N[0] ; i++ )
        {
            ...
            for( ktime = 1; ktime<fpulse.NewNt; ktime++ )
            {
                ...
                for( l = 0; l < Nrk4; l++ )
                {
                    ...
                    tef[l]      = fpulse.a_ef0;//complex( real(fpulse.a_ef0), 0. );
                    taf[l]      = fpulse.a_af0;//complex( real(fpulse.a_af0), 0. );
                ...
                group_rad[0][ktime-1]+=  tgv_v[0]  * nv +  tgv_c[0] * real( rkutta_nc[ktime-1] );
                group_rad[1][ktime-1]+=  tgv_v[1]  * nv +  tgv_c[1] * real( rkutta_nc[ktime-1] );
                
                anomalous_rad[0][ktime-1]+=  cs.xanomalous_v0 * nv + cs.xanomalous_c0 * real( rkutta_nc[ktime-1] );
                anomalous_rad[1][ktime-1]+=  cs.yanomalous_v0 * nv + cs.yanomalous_c0 * real( rkutta_nc[ktime-1] );
                
                
                
                
                intra_rad[0][ktime-1]+= ( tgv_v[0] + cs.xanomalous_v0) * nv + ( tgv_c[0] + cs.xanomalous_c0)* real( rkutta_nc[ktime-1] );
            ...
            if( i%((g.N[0]-1)/4)==0 )
            {
                cout << "\n" << ktime-2 << "   " << g.k[0][i] << "   " << tky[0] << "       " << real(ConductionOccup[ktime-2]*g.dV) << "       " << real(Coherence_Mom_Int[ktime-2])*g.dV << "           "<<cs.gauge << "        rank = "  << rank;
