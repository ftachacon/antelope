...
// Fix: now axis is not always cartesian
// Ndim vectors define the axis
...
public:
    ...
    momaxis( int *_N
            ,double *_axisvec, double *_startk );                // Creator/Constructor Object ...
    ...
    void steps_sizes( int *_N
                     ,double *_axisvec, double *_startk );            // Momentum calculation
...
//Constructor
momaxis::momaxis( int *_N ,double *_axisvec, double *_startk )
{
    steps_sizes( _N, _axisvec, _startk );
...
void  momaxis::steps_sizes(int *_N, double *_axisvec, double *_startk )
{
    ...
    {
        N[i] = _N[i];
        StartPoint[i] = _startk[i];
        for (int j = 0; j < Ndim; ++j)
        {
            AxisVec[i][j] = _axisvec[Ndim*i + j];
        }
    }
    ...
    if (( !(N[2]==1) && (N[1]==1 || N[0]==1) )
    || (!(N[1]==1) && N[0]==1))
    {
        cout << "Check N order, N: " << N[0] << " " << N[1] << " " << N[2];
        exit (EXIT_FAILURE);
    }
    if (N[2] ==1)
    {
        ...
        if ( N[1]==1)
        {
            dV = sqrt(AxisVec[0][0]*AxisVec[0][0] + AxisVec[0][1]*AxisVec[0][1] + AxisVec[0][2]*AxisVec[0][2])/(N[0] -1 );
        }
        // 2D condition
        else
        {
            dV = abs(AxisVec[0][0]*AxisVec[1][1] - AxisVec[0][1]*AxisVec[1][0])/(N[0] -1)/(N[1] -1);
        }
    }
    // 3D condition
    else
    {
        dV = AxisVec[0][0]*AxisVec[1][1]*AxisVec[2][2]
            - AxisVec[0][0]*AxisVec[1][2]*AxisVec[2][1]
            + AxisVec[0][1]*AxisVec[1][2]*AxisVec[2][0]
            - AxisVec[0][1]*AxisVec[1][0]*AxisVec[2][2]
            + AxisVec[0][2]*AxisVec[1][0]*AxisVec[2][1]
            - AxisVec[0][2]*AxisVec[1][1]*AxisVec[2][0];
        if (dV < 0)
        {
            cout << "Check order of axis vector - dv = " << dV << "\n";
            dV = abs(dV);
        }
    }
