    ...
    for ( l = 0; l < g->N[2]; l++ )
        for ( j = 0; j < g->N[1]; j++ )
            for ( i = 0; i < g->N[0]; i++ )
            {
                ...
                pmatrix_cv[0][ g->index(&i,&j,&l) ]    = I*eg0*dip_cv0[0];
                pmatrix_cv[1][ g->index(&i,&j,&l) ]    = I*eg0*dip_cv0[1];
...
void solidstructure::set_memory()
{
    ...
    for (n = 0; n < Ngrad; n++ )
    {
        ...
        pmatrix_cv[n]  = (complex*) malloc( (g->Ntotal) * sizeof( complex ) ) ;
        ...
        memset( pmatrix_cv[n],  0, (g->Ntotal)*sizeof( complex ) );
...
void solidstructure::Set_Of_B_BGrad( double const *kx, double const *ky )
{
    ...
    velocity_cv0[0] =  I * eg0 * dip_cv0[0];//dip_cv0[0];//
    velocity_cv0[1] =  I * eg0 * dip_cv0[1];//dip_cv0[1];//
