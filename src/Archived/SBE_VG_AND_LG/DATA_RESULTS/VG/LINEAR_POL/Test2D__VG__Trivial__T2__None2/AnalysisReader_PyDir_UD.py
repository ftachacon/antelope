...
p1, = plt.plot( t, Efield, 'red', lw=2 );
p2, = plt.plot( t, xJinter/max(abs(xJinter))*max(Efield), 'green', lw = 1.5 );
p3, = plt.plot( t, yJinter/max(abs(yJinter))*max(Efield), 'blue',  lw = 1.5 );
...
plt.xlabel('time (a.u.) ', fontsize=18);
plt.ylabel('Jx/Jy (a.u.) Normalized to Ex ', fontsize=18);
plt.tick_params(labelsize=18);
...
#xJinterMasked = np.hamming(Nt)*xJinter
...
p1, = plt.plot( t, Efield, 'red', lw=2 );
p2, = plt.plot( t, xJintraMasked/max(abs(xJintraMasked))*max(Efield), 'green', lw = 1.5 );
p3, = plt.plot( t, yJintraMasked/max(abs( yJintraMasked))*max(Efield), 'blue',  lw = 1.5 );
...
p2,= plt.plot( t, xJinterMasked/max(abs(xJinterMasked))*max(Efield), 'green', lw = 1 );
p3,= plt.plot( t, yJinterMasked/max(abs(yJinterMasked))*max(Efield), 'blue',  lw = 1 );
...
xFFT_Jinter      = np.fft.fftshift( xFFT_Jinter )*(-1j)*w#*(-1j)*w;
yFFT_Jinter      = np.fft.fftshift( yFFT_Jinter )*(-1j)*w#*(-1j)*w;
...
OutputData      = w/w0
OutputData      = np.concatenate((OutputData,np.power(10.,xSinter ) ),axis=1 );
OutputData      = np.concatenate((OutputData,np.power(10.,xSintra )),axis=1 );
OutputData      = np.concatenate( (OutputData,np.power(10.,xSpectrum)),axis=1 );
...
p1, = plt.plot( w/w0, xSinter, 'r-', lw = 2.);
p2, = plt.plot( w/w0, xSintra, 'b',  lw = 1.5 );
p3, = plt.plot( w/w0, xSpectrum, 'g',  lw = 1.2 );

plt.legend([p1, p2, p3]
           , [r'$a^{(M)}_{er,x}$', r'$a^{(M)}_{ra,x}$', '$a_{tot,x}$']
           ,fontsize=18);
...
yaxmin      = np.log10(hzero);      # controling harmonic-order
yaxmax      = np.log10(1e+5);                   # controling harmonic-order

plt.ylim(yaxmin, yaxmax);
xticks0  = np.arange(1,50,4);
plt.xticks( xticks0 );

yticks0  = [-25.,-20., -15, -10.0, -5, 0, 5 ];
plt.yticks( yticks0 );
...
xaxmax      = 33;   # controling harmonic-order axis limits, u


plt.xlim( xaxmin, xaxmax );
plt.ylim( yaxmin, yaxmax );

fname='/'+str(mparam)+'PaperHarmonicSpectrumCNo'+ str("%.1f"%ChernNo) + 'M_' + str("%.2f"%M0) + 'Phi_' + str("%.2f"%Phi0) + '.png'
...
p1, = plt.plot( w/w0, xSpectrum, 'r-', lw = 2.);
p2, = plt.plot( w/w0, ySpectrum, 'b',  lw = 1.5 );
p3, = plt.plot( w/w0, Spectrum, 'g',  lw = 1.2 );

plt.legend([p1, p2, p3]
           , [r'$a^{(M)}_{x}$', r'$a^{(M)}_{y}$', '$a_{tot}$']
           ,fontsize=18);
...
fname='/'+str(mparam)+'Full__HHG__Spectra__CNo0__M__' + str("%.2f"%M0) + 'Phi_' + str("%.2f"%Phi0) + '.png'
