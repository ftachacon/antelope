...
complex der_pi( double const *eg, double const *xig_ef, double const *T2, complex const *ROmega, complex const *cohPi0, complex *cpi);
double der_nc( complex const *ROmega, complex const *cpi, double *nc );
...
int main( int argc, char *argv[] )
{
    ...
    for( ktime = 0; ktime<fpulse.NewNt; ktime++ )
    {
        ...
        // at tn, tn + dt/2., and, tn + dt; tn = t0 + n*dt
    ...
    for( jtemp = jstart; jtemp < jend; jtemp++ )
    {
        ...
        for( itemp = 0; itemp < g.N[0] ; itemp++ )
        {
            ...
            k_pi[0]=der_pi( &teg[0], &txig[0], &cs.T2, &tROmega[0], &rkutta_pi[g.index( &itemp, &jtemp, &ktemp ) ], &k_pi[0] );
            
            temp_pi0[1] = rkutta_pi[ g.index( &itemp, &jtemp, &ktemp ) ] + k_pi[0]*dt/2.;
            k_pi[1]=der_pi( &teg[1], &txig[1], &cs.T2, &tROmega[1], &temp_pi0[1], &k_pi[1] );
            
            temp_pi0[2]    = rkutta_pi[ g.index( &itemp, &jtemp, &ktemp ) ] + k_pi[1]*dt/2.;
            k_pi[2]=der_pi( &teg[2], &txig[2], &cs.T2, &tROmega[2], &temp_pi0[2], &k_pi[2] );
            
            temp_pi0[3]    = rkutta_pi[ g.index( &itemp, &jtemp, &ktemp ) ] + k_pi[2]*dt;
            k_pi[3]=der_pi( &teg[3], &txig[3], &cs.T2, &tROmega[3], &temp_pi0[3], &k_pi[3] );
            ...
            //####################################################
            //Occupation via RK4
            k_nc[0] = der_nc( &tROmega[0], &rkutta_pi[ g.index( &itemp, &jtemp, &ktemp ) ], &k_nc[0] );
            _half_pi = ( rkutta_pi_ktimeplus + rkutta_pi[ g.index( &itemp, &jtemp, &ktemp ) ] )/2.;
            k_nc[1] = der_nc( &tROmega[1], &_half_pi, &k_nc[1] );
            k_nc[2] = der_nc( &tROmega[2], &_half_pi, &k_nc[2] );
            k_nc[3] = der_nc( &tROmega[3], &rkutta_pi_ktimeplus, &k_nc[3] );
