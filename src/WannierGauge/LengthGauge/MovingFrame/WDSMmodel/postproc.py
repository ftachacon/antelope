...
class postprocessing(object):
    ...
    def plot_t_data(self, data_list=[], label_list=[], save=False, figname=''):
        ...
        for item, label in zip(data_list, label_list):
            plt.plot(self.t, item, lw=2., label=label)
        plt.legend()
        plt.xlabel('time (a.u.) ', fontsize=18)
        plt.ylabel('$J$ (a.u.) ', fontsize=18)
    ...
    def fft(self, data):
        ...
        for i in range(self.dim):
            ...
            fft_current[i, :] = -1j * self.w * np.fft.fftshift(fft_current[i, :])
    ...
    def rotation(self, data):
        ...
        self.spectrum = abs(
            self.radiation[0, :])**2 + abs(self.radiation[1, :])**2 + abs(self.radiation[2, :])**2

    def plot_w_data(self, data_list=[], label_list=[], save=False, figname=''):
        fig = plt.figure(figsize=(self.width, self.hight))
        for item, label in zip(data_list, label_list):
            item = np.log10(item)
            plt.plot(self.w / self.w0, item, lw=2., label=label)
            plt.ylim(min(item), max(item))
