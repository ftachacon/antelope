...
int main( int argc, char *argv[] )
{
    ...
    double eps  = 1.0e-15;      // this is not a regularizatino parameter
    ...
    complex *inter_rad_h[Ngrad];
    complex *intra_rad_h[Ngrad];
    complex *ConductionOccup_h;
    complex *Coherence_Mom_Int_h;
    ...
    FILE *interj_out_h, *intraj_out_h, *occup_out_h;
    ...
    if ( rank == MASTER )
    {
        ...
        interj_out_h       = fopen( "interband_dipole_full_evol_hamiltonian.dat", "w");
        intraj_out_h       = fopen( "intraband_current_full_evol_hamiltonian.dat", "w");
        occup_out_h        = fopen( "occupation__full__evol_hamiltonian.dat", "w");
    ...
    {        
        ...
        setting_complex_memory( &inter_rad_h[itemp], fpulse.Nt+1 );
        setting_complex_memory( &intra_rad_h[itemp], fpulse.Nt+1 );
    ...
    setting_complex_memory( &ConductionOccup_h, fpulse.Nt+1 );
    setting_complex_memory( &Coherence_Mom_Int_h, fpulse.Nt+1 );
	...
	{
		tkx5[ 0 ]      = g.k[0][ itemp ];
		for (jtemp = 0; jtemp < g.N[1]; ++jtemp)
		{
			tky5[0]	= g.k[1][jtemp];
            cs.Set_Of_B_BGrad( &tkx5[ 0 ], &tky5[ 0 ] );
            /*kindex = g.index( &itemp, &jtemp, &ktemp);
            if (abs(cs.b1res0) < eps && abs(cs.b2res0) < eps)
            {
                if (cs.b3res0 < -eps)
                {
                    temp_U[0][0] = 0.;  temp_U[0][1] = 1.;
                    temp_U[1][0] = 1.;  temp_U[1][1] = 0.;
                }
                else
                {
                    temp_U[0][0] = 1.;  temp_U[0][1] = 0.;
                    temp_U[1][0] = 0.;  temp_U[1][1] = 1.;
                }
            }
            else
            {
                ftemp = sqrt(2. * cs.bnorm * (cs.bnorm + cs.b3res0));
                temp_U[0][0] = (cs.b3res0 + cs.bnorm) / ftemp;          temp_U[0][1] = -(cs.b1res0 - I * cs.b2res0) / ftemp;
                temp_U[1][0] = (cs.b1res0 + I * cs.b2res0) / ftemp;     temp_U[1][1] = (cs.b3res0 + cs.bnorm) / ftemp;
            }

            temp_rho[0][0] = 0.0;         temp_rho[0][1] = 0.0;
            temp_rho[1][0] = 0.0;         temp_rho[1][1] = 1.0;
            //temp_pi5[0] = 0.;
            //temp_nc5[0] = 0.;
            // {U rho U^+}_ij => \sum_ab U_ia rho_ab U_bj^+ = \sum_ab rho_ab U_ia U_jb^*
            for (int ii = 0; ii < 2; ++ii)
            {
                for (int jj = 0; jj < 2; ++jj)
                {
                    rkutta_pi[kindex] += temp_rho[ii][jj] * temp_U[0][ii] * conj(temp_U[1][jj]);
                    rkutta_nc[kindex] += real(temp_rho[ii][jj] * temp_U[0][ii] * conj(temp_U[0][jj]) );
                }
            }*/
			rkutta_pi[ g.index( &itemp, &jtemp, &ktemp) ] = - (cs.b1res0 - I*cs.b2res0)/2./cs.bnorm;
			rkutta_nc[ g.index( &itemp, &jtemp, &ktemp) ] = (cs.bnorm -  cs.b3res0)/2./cs.bnorm;
		}
	}
    /*ktemp = 0;
    if (rank == MASTER)
    {
        temp_pi5[0] = 0.;   temp_nc5[0] = 0.;
        cout << "Pi - Nc print \n";
        for (itemp = 0; itemp < g.N[0]; ++itemp)
        {
            for (jtemp = 0; jtemp < g.N[1]; ++jtemp)
            {
                //cout << "i : " << itemp << ", j : " << jtemp << ", nc: " << rkutta_nc[ g.index( &itemp, &jtemp, &ktemp) ] << "\n";
                temp_nc5[0] += rkutta_nc[ g.index( &itemp, &jtemp, &ktemp) ];
                temp_pi5[0] += rkutta_pi[ g.index( &itemp, &jtemp, &ktemp) ];
            }
        }
        cout << "pi_wannier: " << temp_pi5[0] / double(g.Ntotal);
        cout << "\nnc_wannier " << temp_nc5[0] / g.Ntotal;
        cout.flush();
        ...
        for (itemp = 0; itemp < g.N[0]; ++itemp)
        {
            ...
            for (jtemp = 0; jtemp < g.N[1]; ++jtemp)
            {
                ...
                temp_rho[0][0] = rkutta_nc[kindex];               temp_rho[0][1] = rkutta_pi[kindex];
                temp_rho[1][0] = conj(rkutta_pi[kindex]);         temp_rho[1][1] = 1.0 - rkutta_nc[kindex];
                temp_pi5[0] = 0.;
                temp_nc5[0] = 0.;
                // {U^+ rho U}_ij => \sum_ab U^+_ia rho_ab U_bj = \sum_ab rho_ab U_ai^* U_bj
                for (int ii = 0; ii < 2; ++ii)
                {
                    for (int jj = 0; jj < 2; ++jj)
                    {
                        temp_pi5[0] += temp_rho[ii][jj] * conj(temp_U[ii][0]) * temp_U[jj][1];
                        temp_nc5[0] += real(temp_rho[ii][jj] * conj(temp_U[ii][0]) * temp_U[jj][0]);
                    }
                }
                if (real(temp_pi5[0]) > eps || imag(temp_pi5[0]) > eps || temp_nc5[0] > eps)
                {
                    cout << "\n pi_hamiltonian " << temp_pi5[0];
                    cout << ", nc_hamiltonian " << temp_nc5[0];
                    cout << ", kindex:  " << itemp << ", " << jtemp;
                    cout << ", kx: " << tkx5[0] << ", ky: " << tky5[0];
                    cout << ", B1: " << cs.b1res0 << ", B2: " << cs.b2res0 << ", B3: " << cs.b3res0;
                }
            }
        }
        cout << "\n\n pi_hamiltonian " << temp_pi5[0] / double(g.Ntotal);
        cout << "\n nc_hamiltonian " << temp_nc5[0] / g.Ntotal;
        cout.flush();
        exit(1);
    }*/
    ...
    {            
        ...
        for( itemp = 0; itemp < g.N[0] ; itemp++ )
        {
            ...
            temp_rho[0][0] = rkutta_nc[kindex];         temp_rho[0][1] = rkutta_pi[kindex];
            temp_rho[1][0] = conj(rkutta_pi[kindex]);   temp_rho[1][1] = 1. - rkutta_nc[kindex];
            temp_pi5[0] = 0.;   temp_nc5[0] = 0.;
            // {U^+ rho U}_ij => \sum_ab U^+_ia rho_ab U_bj = \sum_ab rho_ab U_ai^* U_bj
            for (int ii = 0; ii < 2; ++ii)
            {
            ...
            inter_rad_h[0][ktime]+= 2.*real( conj( cs.velocity_cv0[0] )*temp_pi5[0])*g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ] ;
            
            
            inter_rad_h[1][ktime]+= 2.*real( conj( cs.velocity_cv0[1] )*temp_pi5[0] )*g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];

            //inter_rad[0][ktime]+= 2.*real( -(cs.xb1der0 + I*cs.xb2der0) - I*cs.vec_a[0][0]*(cs.b1res0 + I*cs.b2res0) )*rkutta_pi[ g.index( &itemp, &jtemp, &ktemp) ] *g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ] ;
            //inter_rad[1][ktime]+= 2.*real( -(cs.yb1der0 + I*cs.yb2der0) - I*cs.vec_a[1][0]*(cs.b1res0 + I*cs.b2res0) )*rkutta_pi[ g.index( &itemp, &jtemp, &ktemp) ] *g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ] ;
            inter_rad[0][ktime]+= 2.*real( -(cs.xb1der0 + I*cs.xb2der0) *rkutta_pi[ g.index( &itemp, &jtemp, &ktemp) ] )*g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ] ;
            inter_rad[1][ktime]+= 2.*real( -(cs.yb1der0 + I*cs.yb2der0) *rkutta_pi[ g.index( &itemp, &jtemp, &ktemp) ] )*g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ] ;
            ...
            group_velocity_c[0] =  cs.group_c0[0]  * temp_nc5[0]  * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            group_velocity_c[1] =  cs.group_c0[1]  * temp_nc5[0]  * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            ...
            intra_rad_h[0][ktime]+= classical_velocity_v[0] + classical_velocity_c[0] ;
            intra_rad_h[1][ktime]+= classical_velocity_v[1] + classical_velocity_c[1] ;
            //intra_rad[0][ktime]+= -(cs.xb0der0 - cs.xb3der0) - 2.*cs.xb3der0 * rkutta_nc[ g.index( &itemp, &jtemp, &ktemp ) ]  * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            //intra_rad[1][ktime]+= -(cs.yb0der0 - cs.yb3der0) - 2.*cs.yb3der0 * rkutta_nc[ g.index( &itemp, &jtemp, &ktemp ) ]  * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            intra_rad[0][ktime]+= ( -(cs.xb0der0 - cs.xb3der0) - 2.*cs.xb3der0 * rkutta_nc[ g.index( &itemp, &jtemp, &ktemp ) ]  )* g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            intra_rad[1][ktime]+= ( -(cs.yb0der0 - cs.yb3der0) - 2.*cs.yb3der0 * rkutta_nc[ g.index( &itemp, &jtemp, &ktemp ) ]  )* g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            //################################
            ...
            ConductionOccup_h[ktime]+=   temp_nc5[0]*g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];            
            Coherence_Mom_Int_h[ktime]+= temp_pi5[0]*g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            //##############################################
            ...
            /*if (abs(k_pi5[0]) > eps)
            {
                cout << "kx : " << tkx5[0] << ", ky: " << tky5[0] << ", k_pi: " << k_pi5[0];
                cout << ", pi: " << temp_pi5[0] << ", nc: " << temp_nc5[0] << "eg: " << teg5[0] << ", romega: " << tROmega5[0] << "\n";
            }*/
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        if (rank == MASTER)
        {
            MPI_Reduce(MPI_IN_PLACE, inter_rad_h[i], fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
        }
        else
        {
            MPI_Reduce(inter_rad_h[i], inter_rad_h[i], fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
        }
        MPI_Barrier( MPI_COMM_WORLD );
        if (rank == MASTER)
        {
            MPI_Reduce(MPI_IN_PLACE, intra_rad_h[i], fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
        }
        else
        {
            MPI_Reduce(intra_rad_h[i], intra_rad_h[i], fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
        }
        MPI_Barrier( MPI_COMM_WORLD );
        if (rank == MASTER)
        {
    ...
    if (rank == MASTER)
    {
        MPI_Reduce(MPI_IN_PLACE, ConductionOccup_h, fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
    }
    else
    {
        MPI_Reduce(ConductionOccup_h, ConductionOccup_h, fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
    }
    MPI_Barrier( MPI_COMM_WORLD );
    // coherence
    if (rank == MASTER)
    ...
    if (rank == MASTER)
    {
        MPI_Reduce(MPI_IN_PLACE, Coherence_Mom_Int_h, fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
    }
    else
    {
        MPI_Reduce(Coherence_Mom_Int_h, Coherence_Mom_Int_h, fpulse.Nt, MPI_DOUBLE_COMPLEX, MPI_CLX_SUM, MASTER, MPI_COMM_WORLD);
    }
    ...
    if (rank == MASTER)
    {
        ...
        for (ktime = 0; ktime < fpulse.Nt; ++ktime)
        {
            ...
            //fprintf(interj_out, "%.16e    %.16e\n", real(inter_rad[0][ktime]) / g.Ntotal, real(inter_rad[1][ktime]) / g.Ntotal);
            //fprintf(intraj_out, "%.16e    %.16e\n", real(intra_rad[0][ktime]) / g.Ntotal, real(intra_rad[1][ktime]) / g.Ntotal);
            fprintf(occup_out, "%.16e\n", real(ConductionOccup[ktime]) / g.Ntotal);

            fprintf(interj_out_h, "%.16e    %.16e\n", real(inter_rad_h[0][ktime]) / g.Ntotal, real(inter_rad_h[1][ktime]) / g.Ntotal);
            fprintf(intraj_out_h, "%.16e    %.16e\n", real(intra_rad_h[0][ktime]) / g.Ntotal, real(intra_rad_h[1][ktime]) / g.Ntotal);
            fprintf(occup_out_h, "%.16e\n", real(ConductionOccup_h[ktime]) / g.Ntotal);
        }
        fflush(interj_out);
        fflush(intraj_out);
        fflush(occup_out);
    ...
    for ( itemp=0; itemp < Ngrad; itemp++)
    {
        ...
        free( intra_rad_h[itemp] );
        free( inter_rad_h[itemp] );
    ...
    free( ConductionOccup_h );
    free( Coherence_Mom_Int_h );
    free( rkutta_nc );
    free( rkutta_pi );
