/// Material property generator for TMDC - 3 band approximation
/**
...
class TMDC : public WannierMaterial
{
public:
    // TB parameters
    ...
    TMDC( const libconfig::Setting *params );
    ~TMDC();
    //void SetBasis();
...
TMDC::TMDC( const libconfig::Setting *params )
{
    isSOC = false;
    ...
    string material = params->lookup("Material");
...
void TMDC::GenInitialValue(complex *_dmstore, std::array<double, Ndim> _kpoint)
{
...
void TMDC::GenUMatrix(complex *_ustore, std::array<double, Ndim> _kpoint)
{
    int num_of_eig; 
    int info = LAPACKE_zheevr( LAPACK_ROW_MAJOR, 'V', 'A', 'U',
                                Nband, tempHmat, Nband, 0., 0., 0., 0., 
...
void TMDC::GenHamiltonian(complex *_hstore, std::array<double, Ndim> _kpoint)
{
    double r3 = sqrt(3.);
...
void TMDC::GenJMatrix(complex **_jstore, std::array<double, Ndim> _kpoint)
{
    // diff components
...
std::tuple<std::array<double, Ndim*Ndim>, std::array<double, Ndim> > TMDC::GenBrillouinzone(  )
{
...
void TMDC::PrintMaterialInformation()
{
    cout << "============================================\n";
