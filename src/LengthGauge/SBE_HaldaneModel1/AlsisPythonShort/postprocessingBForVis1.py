...
inter_dip_ky = np.zeros( (cs.Ntime,2) )
intra_jc_ky  = np.zeros( (cs.Ntime,2) )
occup0       = np.zeros( (cs.Ntime,2) )
occup0[:,0]  = laser[:,0]
...
########################################
for n in set:
    fname0  = 'InterBandIntraBandNx_' + str('%.6d' % cs.Nx) +  '_kyIndex_' + str('%.6d' % n) + '.dat'
...
#occup = np.concatenate( (laser[:,0],occup0[:,0]), axis=1)

np.savetxt( qpath0, inter_dip_ky, '%.16f' );
np.savetxt( qpath1, intra_jc_ky, '%.16f' );
np.savetxt( qpath2, occup0, '%.16f' );
