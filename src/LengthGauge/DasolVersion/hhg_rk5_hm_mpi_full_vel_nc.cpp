...
int main( int argc, char *argv[] )
{
    ...
    string IntionMethod = "NonTrapz";//       // "Trapz" or "Simpson" or "NoneTrapz"/"NoneSimpson"
    ...
    {            
        ...
        for( itemp = 0; itemp < g.N[0] ; itemp++ )
        {
            ...
            group_velocity_v[0] =  (g.k[0][itemp] +cs.group_v0[0])  * nv * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            group_velocity_v[1] =  (g.k[1][jtemp] +cs.group_v0[1])  * nv * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            

            group_velocity_c[0] =  (g.k[0][itemp] +cs.group_c0[0])  * rkutta_nc[ g.index( &itemp, &jtemp, &ktemp ) ]  * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
            group_velocity_c[1] =  (g.k[1][jtemp] +cs.group_c0[1])  * rkutta_nc[ g.index( &itemp, &jtemp, &ktemp ) ]  * g.weight[ g.index(  &itemp, &jtemp, &ktemp ) ];
...
void der_pi( const double  *eg, const double *xig_ef, const double *T2, const complex *ROmega, const complex *cohPi0, const double *_nc, complex *cpi)
{
    ...
    *cpi = -I*( ( *eg + *xig_ef - 0.*I/(*T2) ) *( *cohPi0 ) + (*ROmega)*( 1. - 2.*(*_nc ) )   );
