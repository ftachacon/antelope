...
FileNameIntraInter= '/Intra__Inter__Time__Dependent__Currents__VG__Nx__301__dt__15__ky__0__ContDipole__InterPolarization.dat'
#'/Intra__Inter__Time__Dependent__Currents__VG__Nx__201__dt__10__ky__0__ContDipole__InterMomentum.dat';

FileNameIntraInter1     = FileNameIntraInter
...
xinterC 	= InterC[:,2] - InterC[0,2];
yinterC     = InterC[:,1] - InterC[0,1];

xJintra  	= IntraC[:,1] - IntraC[0,1];
yJintra     = IntraC[:,2] - IntraC[0,2];
...
p1, = plt.plot( te, Efield/max(Efield)*max(xJinter), 'red', lw=2 );
p2, = plt.plot( tj, xJinter, 'green', lw = 1.5 );
p3, = plt.plot( tj, xJintra, 'blue',  lw = 1.5 );
...
ta 	    = tj[0]  + T0*2.0;#T0*3.0;
tb 	    = tj[-1] - T0*2.0;#T0*3.0;
asigma  = T0*0.75
bsigma  = T0*0.75
...
p1, = plt.plot( te, Efield/max(Efield)*max(xJintra), 'red', lw=2 );
p2, = plt.plot( tj, xJintraMasked, 'green', lw = 1.5 );
p3, = plt.plot( tj, xJinterMasked, 'blue',  lw = 1.5 );
#plt.title(title_name,fontsize=18);



plt.legend([p1, p2, p3], ['$E_{L}$', '$J_{x,ra}$', '$J_{x,er}$']);
...
p2,= plt.plot( tj, xJinterMasked, 'green', lw = 1 );
p3,= plt.plot( tj, yJinterMasked, 'blue',  lw = 1 );
...
ofname          = "/HHG__Nx301__E0index__" +str("%.4d"%iparam) + "__.dat"
...
p2, = plt.plot( w/w0, ySinter, 'b',  lw = 1.5 );
p3, = plt.plot( w/w0, xSpectrum, 'g',  lw = 1.2 );

plt.legend([p1, p2, p3]
           , [r'$a^{(P)}_{er}$', r'$a^{(M)}_{ra}$', '$a_{tot}$']
           ,fontsize=18);
...
xaxmax      = 34;   # controling harmonic-order axis limits, u
