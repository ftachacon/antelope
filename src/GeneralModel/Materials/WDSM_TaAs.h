...
// Copied from geonda branch (work of Andrey Geondzhian <andrey.geondzhian@gmail.com>)
...
WDSM_TaAs::WDSM_TaAs( const libconfig::Setting *params )
{
    ...
    {
        std::cerr << "Some WDSM paramters are missing" << std::endl;
        exit(EXIT_FAILURE);
    }
    // int M0=0;
    // SetBasis();
...
void  WDSM_TaAs::GenJMatrix(complex **_jstore, std::array<double, Ndim> _kpoint)
{
    ...
    // std::cout<<"here wdsm\n";
    // std::fill(Bcomp, Bcomp+16, 0.);
    std::fill(xderBcomp, xderBcomp+16, 0.);
    std::fill(yderBcomp, yderBcomp+16, 0.);
    std::fill(zderBcomp, zderBcomp+16, 0.);

    std::fill(xderBcomp_c, xderBcomp_c+16, 0.);
    std::fill(yderBcomp_c, yderBcomp_c+16, 0.);
    std::fill(zderBcomp_c, yderBcomp_c+16, 0.);
    ...
    //     // std::cout<<-xderBcomp[i]-I*xderBcomp_c[i]<<std::endl;
...
{
    std::cout << "============================================\n";
    std::cout << " WDSM_TaAs Model paramters\n";
    std::cout << "a0         = " << a0 << " a.u. \n";
    std::cout << "my         = " << my << " a.u. \n";
    std::cout << "mz         = " << mz << " a.u. \n";
    std::cout << "t          = " << t << " a.u. \n";
...
void  WDSM_TaAs::GenBcomp(std::array<double, Ndim> _kpoint)
{
    ...
    // std::cout<<Bcomp_c[9]<<std::std::endl;
