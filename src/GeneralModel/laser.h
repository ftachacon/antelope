...
// below variables are only for internal data processing or printing
    ...
    std::array<double, Ndim> xdir, ydir;         // direction of laser x-axis and y-axis in real sapce
public:
        ...
        : E0(_E0), w0(_w0), ellip(_ellip), ncycles(_ncycle), cep(_cep), t0(_t0), phix(_phix), envname(_env_name), thetaz(_thetaz), phiz(_phiz)
    {
        ...
        std::array<double, Ndim> trans_x0 = {cos(thetaz)*cos(phiz), cos(thetaz)*sin(phiz), -sin(thetaz)};
        std::array<double, Ndim> trans_y0 = {-sin(phiz), cos(phiz), 0.};
    ...
    std::array<double, Ndim> avlaser(double _t)
    {
    ...
    std::array<double, Ndim> elaser(double _t)
    {
    ...
    {
        std::cout << "-------------------------------------------------------------------\n";
        std::cout << "E0 = " << E0 << ", I0 = " << I0 << "(W/cm^2) , w0 = " << w0 << ", ellip = " << ellip << ", ncycles = " << ncycles << ", cep = " << cep << std::endl;
        std::cout << "t0 = " << t0 << ", phix = " << phix << ", (theta, phi)z = (" << thetaz << ", " << phiz << "), envelope = " << envname << std::endl;
        std::cout << "-------------------------------------------------------------------\n";
    }
};
...
public:
	...
	std::vector<Pulse> pulses;       ///< collection of pulses
    ...
    std::array<double, Ndim> initial_avec, initial_evec;     // store the value at t = atmin
...
void laser::Initialize(double _dt = 0.1, double _blaser = 0., double _alaser = 0.)
{
    ...
    minus0 = *std::min_element(startTime.begin(), startTime.end());
    major0 = *std::max_element(endTime.begin(), endTime.end());
    atmin = minus0 - blaser;
    atmax = major0 + alaser;
...
std::array<double, Ndim> laser::elaser( double _t )
{
    std::array<double, Ndim> outVal = {0., 0., 0.};
    for (auto &el : pulses)
    {
        std::array<double, Ndim> pulseVal = el.elaser(_t);
        for (int i = 0; i < Ndim; ++i) outVal[i] += pulseVal[i];
    }
    for (int i = 0; i < Ndim; ++i) outVal[i] -= initial_evec[i];
    return outVal;
}
std::array<double, Ndim> laser::avlaser( double _t )
{
    std::array<double, Ndim> outVal = {0., 0., 0.};
    for (auto &el : pulses)
    {
        std::array<double, Ndim> pulseVal = el.avlaser(_t);
        for (int i = 0; i < Ndim; ++i) outVal[i] += pulseVal[i];
    }
