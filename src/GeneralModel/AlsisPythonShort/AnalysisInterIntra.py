...
# Fixed output - w, real(jx_inter), imag(jx_inter), real(jy_inter), imag(jy_inter)
#                   real(jx_intra), imag(jx_intra), real(jx_intra), imag(jy_intra), total_HHG
...
for i in range(Dim):
  ...
  OutputData = np.concatenate( (OutputData, np.real(FFT_InterC[i, Nthalf:Nt-1])[:, None] ), axis=1 )
  OutputData = np.concatenate( (OutputData, np.imag(FFT_InterC[i, Nthalf:Nt-1])[:, None] ), axis=1 )
for i in range(Dim):
  OutputData = np.concatenate( (OutputData, np.real(FFT_IntraC[i, Nthalf:Nt-1])[:, None] ), axis=1 )
  OutputData = np.concatenate( (OutputData, np.imag(FFT_IntraC[i, Nthalf:Nt-1])[:, None] ), axis=1 )
