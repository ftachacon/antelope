...
void der_pi( double *eg, double *xig_ef, double *T2, complex *ROmega, complex *nc,  complex *cohPi0, complex *cpi);
...
int main( int argc, char *argv[] )
{
    ...
    for( j = jstart; j < jend; j++ )
    {
        ...
        for( i = 0; i < g.N[0] ; i++ )
        {
            ...
            rkutta_nc[1]          = complex( 0., 0. );
            rkutta_pi[1]          = complex( 0., 0. );
            l                     = 0;
            ...
            for( ktime = 1; ktime<fpulse.NewNt-1; ktime++ )
            {
                ...
                //tROmega[l]  = real( tef[l] ) + complex(0., 1.) * imag( tef[l] ) ;
                
                
                //####################################################################
                //Coherence of the occupations via Finite Difference Method order FDM
                der_pi( &teg[0], &txig[0], &cs.T2, &tROmega[0], &rkutta_nc[ktime], &rkutta_pi[ktime], &k_pi[0] );
                rkutta_pi[ktime+1] = rkutta_pi[ktime-1] + 2. * dt * k_pi[0];
                ...
                rkutta_nc[ktime+1] = rkutta_nc[ktime-1] + 2. * dt * k_nc[0];
                ...
                inter_rad[0][ktime]+= 2.*real( conj( cs.dip_cv0[0] )*rkutta_pi[ktime] );
                inter_rad[1][ktime]+= 2.*real( conj( cs.dip_cv0[1] )*rkutta_pi[ktime] );
                ...
                cs.anomalous_velCV( tef[l], cs.zBcurva_c0 );
                nv = 1. -  real( rkutta_nc[ktime] );
                
                
                group_rad[0][ktime]+= cs.group_v0[0]  * nv +  cs.group_c0[0] * real( rkutta_nc[ktime] );
                group_rad[1][ktime]+= cs.group_v0[1]  * nv +  cs.group_c0[1] * real( rkutta_nc[ktime] );
                ...
                intra_rad[0][ktime]+= ( cs.group_v0[0] - cs.xanomalous_v0 ) * nv + ( cs.group_c0[0] - cs.xanomalous_c0)* real( rkutta_nc[ktime] );
                intra_rad[1][ktime]+= ( cs.group_v0[1] - cs.yanomalous_v0 ) * nv + ( cs.group_c0[1] - cs.yanomalous_c0 ) * real( rkutta_nc[ktime] );
...
void der_pi( double *eg, double *xig_ef, double *T2, complex *ROmega, complex *nc, complex *cohPi0, complex *cpi)
{
    ...
    *cpi = -I*(
               ( *eg + *xig_ef - I/(*T2) ) *( *cohPi0 )
               + (*ROmega)*( 1.- 2.*real( *nc )*1. )
               )  ;
