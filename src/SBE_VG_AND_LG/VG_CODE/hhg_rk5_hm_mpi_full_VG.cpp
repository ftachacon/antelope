...
int main( int argc, char *argv[] )
{
     ...
     ****/
    double la0            = 1.0;   //1.446;         //Lattice constant in Angstroms
    double a0             = la0/0.529;    //Lattice constant in atomic units (a.u.)
    double t1             = 0.075;      //0.107353; //NN Hopping parameters in a.u.
    double t2             = t1/3.;      //0.;//      //Amplitude of the Next Neirest Neibour of HM
    double phi0           = iparam;     //Magnetic flux or phase of the NNN hoping parameter
    double M0             = jparam*t2; //jparam  //Local or on-site potential
    double chernN0        = 0.;
    ...
    double wfreq        = 0.014;         //0.015194118;// Frequency in atomic units (a.u.)
...
void der_pi( const double  *eg, const double *xig_ef, const double *T2, const complex *ROmega, const complex *cohPi0, const double *_nc, complex *cpi)
{
    ...
    *cpi = -I*( ( *eg + *xig_ef - I/(*T2) * flag_coh_dephasing ) *( *cohPi0 ) + (*ROmega)*( 1. - 2.*(*_nc ) )   );
