...
nflag0                  = str("%.5d"%cs.Nx)+ "_kyIndex_" + str( "%.5d"%jparam ) + ".bin"; # file flag for ky-index parameter and binary data type
nflag2                  = str("%.5d"%cs.Nx)+ "_kyIndex_" + str( "%.5d"%jparam ) + ".dat"  # file flag for ky-index parameter and *.dat data type
...
rdata1           = np.fromfile( path1, dtype = np.dtype("f8") ) #loading real part of coherence
...
######################################################
####
interDip        = np.zeros( (cs.Ntime-1,2) );
intraJ          = np.zeros( (cs.Ntime-1,2) );
# jvg[0] = nc*vg_c[0] + nv*vg_v[0]
for ktime in range( cs.Ntime - 1 ):
