...
t			= Laser[0:,1];#InterC[:,1]#Laser[index1,0];
Efield		= Laser[0:,2];#InterC[:,2]#Laser[index1,3];
...
#ValPopulation   = ValPopk[:,2];
#CondPopulation  = ValPopk[:,3];
...
xJinter[0:-1] 	    = np.diff( xinterC )/dt;
yJinter[0:-1]       = np.diff( yinterC )/dt;
...
#
#
##plt.plot( t, 1.-ValPopulation, 'red', lw=1 );
##plt.plot( t, CondPopulation, 'blue', lw=1 );
##plt.title(title_name,fontsize=18);
#
#
#
#plt.xlabel('time (a.u.) ', fontsize=18);
#plt.ylabel('Populations/val,cond  ', fontsize=18);
#plt.tick_params( labelsize = 18 );
#
#
#
...
#
#
#filename0           = FigureDir + '/PopulationOs.pdf';
#fileNamePicture     = ProjPath + filename0;
#
#
#plt.savefig(fileNamePicture);
...
#p1,= plt.plot( t, Efield, 'red', lw=1.25 );
...
#plt.title(title_name,fontsize=18);
#plt.legend([p1, p2, p3], ['$E_{L}$', '$J_{er}$', '$J_{ra}$']);
...
#xFullRadiation      = xFFT_Jinter
#yFullRadiation      = yFFT_Jinter
...
#intra-band current spectrum
#inter-band current spectrum
...
#full current radiation emission
#full current radiation emission
#####################################################
...
p1, = plt.plot( w/w0, xSpectrum, 'blue', lw = 2., label='$J_{x}$' );
p2, = plt.plot( w/w0, ySpectrum, 'r-',  lw = 1.5, label='$J_{y}$' );

#p1, = plt.plot( w/w0, xSinter, 'blue', lw = 2.5, label='$J_{x,er}$' );
#p2, = plt.plot( w/w0, ySinter, 'red',  lw = 2.5, label='$J_{y,er}$' );


#p3, = plt.plot( w/w0, xSintra, 'black', lw = 2., label='$J_{x,ra}$' );
#p4, = plt.plot( w/w0, ySintra, 'm',  lw = 2., label='$J_{y,ra}$' );

#p1, = plt.plot( w/w0, Spectrum, 'green', lw=2.8, label='$J_{tot}$' );

#plt.legend([p1, p2, p3, p4], ['$J_{x,er}$', '$J_{y,er}$', '$J_{x,ra}$', '$J_{y,ra}$'],fontsize=18);
...
yp = -1.1
    ...
    #if (mparam=='x'):
    #plt.text(2.5,-0.5,'a); '+ r'   $T_2=\infty$;  ' + '$E_0= $'+str("{0:.4f}".format(E0)) + ' a.u.',fontsize=18);
#    plt.text(xp,yp, 'a) Par. Emission' , fontsize=28 );

#if (mparam=='y'):
#    plt.text(xp,yp, 'b) Per. Emission ' , fontsize=28 );
...
yticks0  = [-15., -12.5, -10.0, -7.5, -5, -2.5, 0.0, 2.5, 5.0];
plt.yticks( yticks0 );
